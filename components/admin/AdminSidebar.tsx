    ...
    <div className="flex h-full flex-col">
      <div className="border-b border-brand-100 px-5 py-5">
        <Link href="/admin" onClick={onClose} className="flex items-center gap-3 rounded-xl">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-charcoal-600 shadow-sm">
            <Shield className="h-5 w-5 text-white" />
          </div>
          <div>
            <span className="block text-lg font-extrabold text-charcoal-600 tracking-tight">GeoVerse</span>
            <span className="text-xs font-semibold text-brand-600">Admin Panel</span>
          </div>
        </Link>
      </div>
      ...
      <div className="border-t border-brand-100 p-4">
        <Link
          href="/dashboard"
          onClick={onClose}
          className="flex items-center gap-3 rounded-xl px-4 py-2.5 text-sm font-semibold text-stone-400 transition-all hover:bg-brand-50 hover:text-brand-700"
        >
...
export default function AdminSidebar({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-background via-white to-brand-50/30">
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-72 flex-col border-r border-brand-100 bg-white/95 shadow-sm backdrop-blur lg:flex">
      ...
      <div className="fixed left-0 right-0 top-0 z-50 flex items-center justify-between border-b border-brand-100 bg-white/95 px-4 py-3 shadow-sm backdrop-blur lg:hidden">
        <div className="flex items-center gap-2">
          <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-charcoal-600">
            <Shield className="h-4 w-4 text-white" />
          </div>
          <span className="text-lg font-extrabold text-charcoal-600">Admin</span>
        </div>
        <button
      ...
      <AnimatePresence>
        {mobileOpen && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 0.3 }} exit={{ opacity: 0 }} onClick={() => setMobileOpen(false)} className="fixed inset-0 z-40 bg-charcoal-700 lg:hidden" />
